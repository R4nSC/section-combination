...
def create_dataset_folder_for_ensemble(params, logger):
    ...
    #     # 保存するディレクトリがなければ作成する
    #     os.makedirs(os.path.join(ensemble_data_path, section_name), exist_ok=True)
    #     # 指定セクションの単体画像のImageFolderを作成
    #     single_image_folder = ImageFolderWithPaths(os.path.join(single_data_path, section_name), params.transforms)

    #     # ImageFolderから画像を1つずつ取り出して.text/.rdata/.dataの3セクションをすべて含んでいるか判定
    #     for i in range(len(single_image_folder)):
    #         # 画像のパスを取得
    #         single_image_path = single_image_folder[i][2]

    #         # '/'で文字列を区切り，ファイル名とラベル（ファミリー名）を取得
    #         path_split_by_slash = single_image_path.split('/')
    #         filename = path_split_by_slash[-1]
    #         label = path_split_by_slash[-2]

    #         # 指定サンプルに含まれたセクション名をすべて列挙する
    #         list_of_section_names_included_sample = get_section_name(filename[:-4], label)
    ...
    #                         os.path.join(ensemble_data_path, section_name, label, filename))

    #             # 対象セクションを含むサンプルの元画像(allSection)をコピーする
    #             # ※1度だけ実行したい→textセクションを処理しているときだけコピー処理を行う
    #             if section_name == 'text':
    #                 # 元画像が存在するパス
    #                 original_image_path = single_image_path.replace("text", "allSection")

    #                 # 保存先ディレクトリの確保
    #                 if not os.path.exists(params.config['data_dir']['ensemble'] + section_name):
    #                     os.makedirs(params.config['data_dir']['ensemble'] + section_name)
    #                 if not os.path.exists(params.config['data_dir']['ensemble'] + '/allSection/' + label):
    #                     os.makedirs(params.config['data_dir']['ensemble'] + '/allSection/' + label)

    #                 # 対象画像ファイルをコピーする
    #                 shutil.copy(original_image_path,
    #                             params.config['data_dir']['ensemble'] + '/allSection/' + label + '/' + filename)
